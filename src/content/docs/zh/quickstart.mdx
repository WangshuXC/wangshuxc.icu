...
1. **创建 GitHub OAuth 应用：**
   ...
   - 创建新的 OAuth 应用
   - 设置授权回调 URL 为：`http://localhost:3050/api/auth/callback/github`
...
1. **创建 Google OAuth 应用：**
   ...
   - 启用 Google+ API
   - 创建 OAuth 2.0 凭据
   - 设置重定向 URI 为：`http://localhost:3050/api/auth/callback/google`
...
访问 `http://localhost:3050/login` 进行测试：
- ✅ **邮箱/密码登录** - 创建账户并登录
...
在 `http://localhost:3050/dashboard` 访问仪表板：
- ✅ **用户仪表板** - 个人用户界面
- ✅ **管理员仪表板** - 管理控制（管理员用户）
- ✅ **文件管理** - 上传和管理文件
- ✅ **用户管理** - 管理员用户管理界面
...
在 `http://localhost:3050/pricing` 测试支付流程：
- ✅ **订阅计划** - 查看可用的定价计划
...
在 `http://localhost:3050/dashboard/files` 尝试文件操作：
- ✅ **文件上传** - 拖放文件上传
- ✅ **图像处理** - 自动缩略图生成
- ✅ **文件组织** - 组织和管理文件
- ✅ **访问控制** - 文件权限和共享
